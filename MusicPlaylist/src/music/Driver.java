...
/**
 * This class is designed to run student implemented methods from the Library class in the console.
 *
 * @author Jeremy Hui
 * @author Vian Miranda
 */
public class Driver {
  /**
   * NOTE TO STUDENTS: This is the seed which the Driver uses when shuffling your playlist. Feel
   * free to change the seed to your choice, but remember that changing the seed will result in
   * different outputs.
   */
  ...
  public static void main(String[] args) {
    ...
    do {
      ...
      for (int ii = 0; ii < methods.length; ii++) StdOut.printf("%d. %s\n", ii + 1, methods[ii]);
      ...
      for (int ii = 0; ii < 2; ii++) StdOut.printf("%d. %s\n", ii + 1, options[ii]);
      StdOut.print("Enter a number => ");
  ...
  public static void testLoadAllPlaylists() {
    ...
    for (int ii = 0; ii < fnSize; ii++) fn[ii] = filenames.get(ii);
  ...
  public static void testInsertSong() {
    ...
    if (wavCheck == 'y' || wavCheck == 'Y') {
      StdOut.print(
          "Enter the name of the file (make sure the file is in '"
              + song.getDirectory()
              + "') => ");
  ...
  public static void printLibrary() {
    if (songLibrary.getPlaylists() == null) StdOut.println("\nYour library is empty!");
    else songLibrary.printLibrary();
  }
}
