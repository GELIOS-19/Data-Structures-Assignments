...
/**
 * This class is designed to run student implemented methods from the Library
...
public class Driver {
  ...
  public static void main(String[] args) {
    ...
    String[] methods = { "addPlaylist", "removePlaylist", "loadAllPlaylists",
        "insertSong", "removeSong", "reversePlaylist", "mergePlaylists",
        "shufflePlaylist", "sortPlaylist", "playPlaylist", "printLibrary" };
  ...
  public static void testInsertSong() {
    ...
    StdOut.print(
        "Do you have a music file that you would like to use (Y/N)? => ");
    char wavCheck = scanner.nextLine().charAt(0);
    if (wavCheck == 'y' || wavCheck == 'Y') {
      StdOut.print("Enter the name of the file (make sure the file is in '"
          + song.getDirectory() + "') => ");
    ...
    if (res)
      StdOut.print("New Playlist: ");
    else
      StdOut.print("Playlist Without Changes: ");
  ...
  public static void testPlayPlaylist() {
    ...
    int playlistIndex = StdIn.readInt();
    StdOut.print(
        "Enter the number of times to play the playlist (minimum 1 time) => ");
