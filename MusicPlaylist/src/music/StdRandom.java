...
/**
 * <b>Overview.</b> The {@code StdRandom} class provides static methods for
 * generating random number from various discrete and continuous distributions,
 * including uniform, Bernoulli, geometric, Gaussian, exponential, Pareto,
 * Poisson, and Cauchy. It also provides method for shuffling an array or
 * subarray and generating random permutations.
 * <p>
 * <b>Conventions.</b> By convention, all intervals are half open. For example,
 * <code>
 * uniformDouble(-1.0, 1.0)</code> returns a random number between
 * <code>-1.0</code> (inclusive) and <code>1.0</code> (exclusive). Similarly,
 * <code>shuffle(a, lo, hi)</code> shuffles the <code>
 * hi - lo</code> elements in the array <code>a[]</code>, starting at index
 * <code>lo</code> (inclusive) and ending at index <code>hi</code> (exclusive).
 * <p>
 * <b>Performance.</b> The methods all take constant expected time, except those
 * that involve arrays. The <em>shuffle</em> method takes time linear in the
 * subarray to be shuffled; the <em>discrete</em> methods take time linear in
 * the length of the argument array.
 * <p>
 * <b>Additional information.</b> For additional documentation, see
...
public final class StdRandom {
   ...
   * probability 1/2.
   *
   * @return {@code true} with probability 1/2 and {@code false} with
   *         probability 1/2
   */
   ...
   * @throws IllegalArgumentException unless {@code p >= 0.0} and
   *                                    {@code p <= 1.0}
   */
  public static int geometric(double p) {
    if (!(p >= 0)) {
      throw new IllegalArgumentException(
          "probability p must be greater than 0: " + p);
    }
    if (!(p <= 1.0)) {
      throw new IllegalArgumentException(
          "probability p must not be larger than 1: " + p);
    }
