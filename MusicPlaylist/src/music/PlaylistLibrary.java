 ...
 *
 * <p>An ArrayList of Playlist objects represents the various playlists within
 * one's library.
 *
 * @author Jeremy Hui
 * @author Vian Miranda
...
public class PlaylistLibrary {
  ...
  public PlaylistLibrary() { this(null); }
   ...
   *
   * <p>2. Declare a reference that will refer to the last song of the circular
   * linked list.
   *
   * <p>3. While there are still lines in the input file: 1. read a song from
   * the file 2. create an object of type Song with the song information 3.
   * Create a SongNode object that holds the Song object from step 3.2. 4.
   * insert the Song object at the END of the circular linked list (use the
   * reference from step 2) 5. increase the count of the number of songs
   *
   * <p>4. Create a Playlist object with the reference from step (2) and the
   * number of songs in the playlist
   *
   * <p>5. Return the Playlist object
   *
   * <p>Each line of the input file has the following format:
   * songName,artist,year,popularity,link
   *
   * <p>To read a line, use StdIn.readline(), then use .split(",") to extract
   * fields from the line.
   *
   * <p>If the playlist is empty, return a Playlist object with null for its
   * last, and 0 for its size.
   *
   ...
   *
   * <p>1. Calls createPlayList() with a file containing song information. 2.
   * Adds the new playlist created by createPlayList() into the songLibrary.
   *
   ...
   * you.
   *
   * <p>It takes a playlistIndex, and removes the playlist located at that
   * index.
   *
   ...
   *
   * <p>The playlist will have the same index in songLibrary as it has in the
   ...
   *
   * <p>The first node of the circular linked list is at position 1, the second
   * node is at position 2 and so forth.
   *
   * <p>Return true if the song can be added at the given position within the
   * specified playlist (and thus has been added to the playlist), false
   * otherwise (and the song will not be added).
   *
   * <p>Increment the size of the playlist if the song has been successfully
   * added to the playlist.
   *
   * @param playlistIndex the index where the playlist will be added
   * @param position the position in the playlist to which the song is to be
   *     added
   ...
   *
   * <p>Use the .equals() method of the Song class to check if an element of the
   * circular linkedlist matches the specified song.
   *
   * <p>Return true if the song is found in the playlist (and thus has been
   * removed), false otherwise (and thus nothing is removed).
   *
   * <p>Decrease the playlist size by one if the song has been successfully
   ...
   *
   * <p>Each node in the circular linkedlist will point to the element that came
   * before it.
   *
   * <p>After the list is reversed, the playlist located at playlistIndex will
   ...
   *
   * <p>Both playlists have songs in decreasing popularity order. The resulting
   * playlist will also be in decreasing popularity order.
   *
   * <p>You may assume both playlists are already in decreasing popularity
   * order. If the songs have the same popularity, add the song from the
   * playlist with the lower playlistIndex first.
   *
   * <p>After the lists have been merged: - store the merged playlist at the
   * lower playlistIndex - remove playlist at the higher playlistIndex
   *
  ...
  public void mergePlaylists(int playlistIndex1, int playlistIndex2) {
    Playlist lowPlaylist =
        this.songLibrary.get(Integer.min(playlistIndex1, playlistIndex2));
    Playlist highPlaylist =
        this.songLibrary.get(Integer.max(playlistIndex1, playlistIndex2));
    ...
    while (lowPlaylist.getSize() != 0 && highPlaylist.getSize() != 0) {
      ...
      Playlist targetPlaylist = lowHead.getSong().getPopularity() >=
                                        highHead.getSong().getPopularity()
                                    ? lowPlaylist
                                    : highPlaylist;

      SongNode targetHead =
          new SongNode(targetPlaylist.getLast().getNext().getSong(), null);
    ...
    Playlist nonEmptyPlaylist =
        lowPlaylist.getSize() == 0 ? highPlaylist : lowPlaylist;
   ...
   *
   * <p>2. While the size of the original playlist is not 0, randomly generate a
   * number using StdRandom.uniformInt(1, size+1). Size contains the current
   * number of items in the original playlist.
   *
   * <p>3. Remove the corresponding node from the original playlist and insert
   ...
   *
   * <p>Set the playlist located at the corresponding playlistIndex in
   * decreasing popularity index order.
   *
   * <p>This method should use a sort that has O(nlogn), such as with merge
   * sort.
   *
  ...
  public void playPlaylist(int playlistIndex, int repeats) {
    ...
    SongNode ptr = songLibrary.get(playlistIndex).getLast().getNext(), first =
                                                                           ptr;
  ...
  public void printPlaylist(int playlistIndex) {
    ...
    for (ptr = songLibrary.get(playlistIndex).getLast().getNext();
         ptr != songLibrary.get(playlistIndex).getLast(); ptr = ptr.getNext()) {
