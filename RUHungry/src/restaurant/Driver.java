...
public class Driver {
  ...
  public static void main(String[] args) {
    ...
    String[] methodsToTest = { "Menu", "StockRoom", "Delete StockNode",
    ...
    do {
      ...
      case 4:
        ...
        if (yesORno.equalsIgnoreCase("Y")) {
          StdOut.println("Alrighty! Which one would you like to test?"); // we
                                                                         // can
          // make
          // separate input
          // files for peeps
          ...
          case 2:
            StdOut
                .print("Slay Queen (or King)! Enter a donation input file => ");
  ...
  public static void testStockHashtable(String inputFile, RUHungry restaurant) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      while (ptr != null) {
        StdOut.print(ptr.getIngredient().getName() + "  ID: "
            + ptr.getIngredient().getID() + " Price: "
            + ((Math.round(ptr.getIngredient().getCost() * 100.0)) / 100.0)
            + " Stock Level: " + ptr.getIngredient().getStockLevel());
        StdOut.println();
    ...
    StdOut
        .println("Here's the menu again! Make sure to check price and profit:");
    StdOut.println();
    for (int i = 0; i < category.length; i++) {
      StdOut.print(category[i] + ":");
      StdOut.println();
  ...
  public static void testDelete(RUHungry restaurant,
    ...
    for (int i = 0; i < 10; i++) {
      ...
      while (ptr != null) {
        StdOut.print(ptr.getIngredient().getName() + "  ID: "
            + ptr.getIngredient().getID() + " Price: "
            + ptr.getIngredient().getCost() + " Stock Level: "
  ...
  // the methods to test transaction methods create a simple
  // TransactionNode and adds it to the transaction LL so that you will be
  // able to run these methods

  public static void testIndividualMethods(String inputOrdersFile,
      String transactionType, RUHungry restaurant) {
    restaurant.createStockHashTable("stock.in");
    restaurant.menu("menu.in");
    restaurant.updatePriceAndProfit();
    ...
    if (transactionType.equalsIgnoreCase("order")) {
      ...
      restaurant
          .addTransactionNode(new TransactionData("N/A", "N/A", 0, 112, false));
      for (int orderNumber = 0; orderNumber < numOrders; orderNumber++) {
        int amountOrdered = StdIn.readInt();
        StdIn.readChar();
        String itemOrdered = StdIn.readLine();
    ...
    while (ptr != null) {
      ...
      StdOut.println("Type: " + type + ", Name: " + item + ", Amount: " + amount
    ...
    StdOut.println("Total profit remaining: $"
        + ((Math.round(restaurant.profit() * 100.0)) / 100.0));
  }
  ...
  public static void testTransactionMethods(String inputOrdersFile,
    ...
    for (int orderNumber = 0; orderNumber < numOrders; orderNumber++) {
      String type = StdIn.readString(); // can be either "order"
      // "donation" "restock"
      StdIn.readChar(); // empty
      int amountOrdered = StdIn.readInt(); // reads how much was
      // ordered/restocked
      StdIn.readChar();
      String itemOrdered = StdIn.readLine(); // reads what item was
      // ordered/requested/delivered
  ...
  public static void testSeatAllGuests(String inputSeatFile,
      String inputTableFile, RUHungry restaurant) {
