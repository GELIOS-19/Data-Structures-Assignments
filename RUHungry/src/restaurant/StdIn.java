 ...
 * put a copy in your working directory. <p> <b>Reading tokens from standard
 * input and converting to numbers and strings.</b> You can use the following
 * methods to read numbers, strings, and booleans from standard input one at a
 * time: <ul> <li> {@link #isEmpty()} <li> {@link #readInt()} <li> {@link
 * #readDouble()} <li> {@link #readString()} <li> {@link #readShort()} <li>
 * {@link #readLong()} <li> {@link #readFloat()} <li> {@link #readByte()} <li>
 * {@link #readBoolean()}
 *  </ul>
 *  <p>
 ...
 * input, one at a time, and prints them one per line. <pre> while
 * (!StdIn.isEmpty()) { double value = StdIn.readDouble();
 ...
 * one at a time: <ul> <li> {@link #hasNextChar()} <li> {@link #readChar()}
 *  </ul>
 *  <p>
 ...
 * on standard input (possibly a whitespace character). <p> As an example, the
 * following code fragment reads characters from standard input, one character
 * at a time, and prints it to standard output. <pre> while
 * (StdIn.hasNextChar()) { char c = StdIn.readChar(); StdOut.print(c);
 *  }
 *  </pre>
 *  <p>
 ...
 * trailing line separator. <p> A <em>line separator</em> is defined to be one
 ...
 *       for consistency with {@link StdOut}, {@link
 * Double#parseDouble(String)}, and floating-point literals. <li> {@code StdIn}
 * has convenient methods for reading a single character; reading in sequences
 ...
 *  {@code Scanner} was introduced, this class was re-implemented to use {@code
 * Scanner}. <p> <b>Using standard input.</b> Standard input is a fundamental
 * operating system abstraction on Mac OS X, Windows, and Linux. The methods in
 * {@code StdIn} are <em>blocking</em>, which means that they will wait until
 * you enter input on standard input. If your program has a loop that repeats
 * until standard input is empty, you must signal that the input is finished. To
 * do so, depending on your operating system and IDE, use either {@code
 * <Ctrl-d>} or {@code <Ctrl-z>}, on its own line. If you are redirecting
 * standard input from a file, you will not need to do anything to signal that
 * the input is finished. <p> <b>Known bugs.</b> Java's UTF-8 encoding does not
 * recognize the optional <a href =
 ...
 * StdIn} will have an extra character {@code \}{@code uFEFF} at the beginning.
 *  <p>
 *  <b>Reference.</b>
