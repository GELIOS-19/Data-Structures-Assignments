...
public class RUHungry {
  ...
  private MenuNode[]
      menuVar; // array of lists of MenuNodes where each index is a category.

  // Stock: hashtable using chaining to resolve collisions.
  private StockNode[]
      stockVar; // array of linked lists of StockNodes (use hashfunction to
  ...
  private TransactionNode
      transactionVar; // refers to the first front node in linked list
   ...
   * b) the next line states the name of the category (populate CategoryVar as
   * you read each category name) c) the next number represents how many dishes
   * are in that category - this will be the size of the linked list in menuVar
   * for this category d) the next line states the name of the dish e) the first
   * number in the next line represents how many ingredient IDs there are f) the
   * next few numbers (all in the 100s) are each the ingredient ID
   *
   * 2. As you read through the input file:
   * a) populate the categoryVar array
   * b) populate menuVar depending on which index (aka which category) you are
   * in i) make a dish object (with filled parameters -- don't worry about
   * "price" and "profit" in the dish object for right now) ii) create menuNode
   * and insert at the front of menuVar (NOTE! there will be multiple menuNodes
   * in one index)
   *
  ...
  private int hash(int ingredientID) { return ingredientID % stockVarSize; }
   ...
   * linked list
   * b) this is NOT efficient. Hashtables are not good if you can't use the key
   * to find the item.
   *
   ...
   * (a) this is an efficient search as it looks only at the linked list which
   * the key hash to
   ...
   * a) first integer (on line 1) is the size of StockVar *** update
   * stockVarSize AND create the stockVar array *** b) first integer of next
   * line represents the ingredientID i) example: 101 on line 2 c) use
   * StdIn.readChar() to get rid of the space between the id and the name d) the
   * string that follows is the ingredient name (NOTE! --> there are spaces
  ...
  public void createStockHashTable(String inputFile) {
    ...
    while (!StdIn.isEmpty()) {
      ...
      String name =
          String.join(" ", Arrays.copyOfRange(firstLine, 1, firstLine.length))
              .trim();
  ...
  public void
  addTransactionNode(TransactionData data) { // method adds new transactionNode
                                             // to the end of LL
   ...
   * linked list:
   * - if the dish that comes right after the dishName can be prepared, great.
   * If not, try the next one and so on.
   * - you might have to traverse through the entire category searching for a
   * dish that can be prepared. If you reach the end of the list, start from the
   ...
   * - Note: the next dish the customer chooses is always the one that comes
   * right after the one that could not be prepared.
   *
   *
   ...
   *
   * ***************
   * This method simulates seating guests at tables. You are guaranteed to be
