...
public class RUHungry {
   ...
   * Appetizers). If index 0 at categoryVar is Appetizers then menuVar at index 0 contains MenuNodes
   * of appetizer dishes.
   *
   ...
   * in that category - this will be the size of the linked list in menuVar for this category d) the
   * next line states the name of the dish e) the first number in the next line represents how many
   * ingredient IDs there are f) the next few numbers (all in the 100s) are each the ingredient ID
   *
   * <p>2. As you read through the input file: a) populate the categoryVar array b) populate menuVar
   * depending on which index (aka which category) you are in i) make a dish object (with filled
   * parameters -- don't worry about "price" and "profit" in the dish object for right now) ii)
   * create menuNode and insert at the front of menuVar (NOTE! there will be multiple menuNodes in
   * one index)
   *
   ...
   * NOT null: find the ingredient and add the given stock amount to the current stock amount b) the
   * ingredientID is NOT -1: find the ingredient and add the given stock amount to the current stock
   * amount
   *
   ...
   *
   * <p>*********** This method goes over menuVar to update the price and profit of each dish, using
   * the stockVar hashtable to lookup for ingredient's costs.
   *
  ...
  /**
   * PICK UP LINE OF THE METHOD: *are you a decimal? cuz the thought of you always floats in my head
   * and the two of use would make double*
   *
   * <p>************ This method initializes and populates stockVar which is a hashtable where each
   * index contains a linked list with StockNodes.
   *
   ...
   *
   * <p>2. create a Ingredient object followed by a StockNode then add to stockVar HINT! --> you may
   * use previous methods written to help you!
   *
  ...
  /**
   * This method adds a TransactionNode to the END of the transactions linked list. The front of the
   * list is transactionVar.
   *
   ...
   *
   * <p>3. return boolean based on whether you can sell the dish or not HINT! --> once you determine
   * you can't sell the dish, break and return
   *
   ...
   *
   * <p>If the dish can be prepared - create a TransactionData object of type "order" where the item
   * is the dishName, the amount is the quantity being ordered, and profit is the dish profit
   ...
   * item is the dishName, the amount is the quantity being ordered, and profit is 0 (zero). - then
   * add the transaction as an UNsuccessful transaction and, - simulate the customer trying to order
   * other dishes in the same category linked list: - if the dish that comes right after the
   * dishName can be prepared, great. If not, try the next one and so on. - you might have to
   ...
   * next dish the customer chooses is always the one that comes right after the one that could not
   * be prepared.
   *
  ...
  public void order(String dishName, int quantity) {
    ...
    if (this.checkDishAvailability(dishName, quantity)) {
      TransactionData transaction =
          new TransactionData("order", dishName, quantity, dish.getProfit() * quantity, true);
   ...
   *
   * <p>The profit is computed by traversing the transaction linked list (transactionVar) adding up
   * all the profits for the day
   *
   ...
   * much each ingredient costs) * (quantity) b) if there is enough profit, adjust stock and profit
   * accordingly
   *
   * <p>2. add transaction to transactionVar
   *
   ...
   *
   * <p>First row of tables[][]: contains the total number of seats available at a table (each table
   * is an index in first row) Second row of tables[][]: initializes all indices to 0 0 --> table is
   * available 1 --> table is occupied
   *
   * <p>** GIVEN METHOD ** ** DO NOT EDIT **
   *
   ...
   * party in the first available table that fits their party. - If there is no available table for
   * the next party, kick a party out from the tables array: 1. starting at index 0 (zero), find the
   * first table big enough to hold the next party in line. 2. remove the current party, add them to
   * the leftQueueVar. 3. seat the next party in line.
   *
   * <p>tableInfo contains the number of seats per table as well as if the table is occupied or not.
   ...
   *
   * @param waitingQueue - queue containing people waiting to be seated (each element in queue is a
   *     People <-- you are given this class!)
   */
