...
public class WeightedQuickUnionUF {
  ...
  /**
   * Since this is a Quick-Union implementation, this method iteratively goes up
   * the parent nodes until arriving at the root node (curr == currParent).
   * <p>
  ...
  public GridLocation find(GridLocation cell) {
    ...
    while (cell.hashCode() != currParent.hashCode()
        || !cell.equals(currParent)) {
   ...
   * <p>
   * Find the root parents of the cell parameters, find the larger branch, set
   * the the smaller branch as the child of the larger branch, then set the size
   * of the larger branch as the sum of both branches.
   * <p>
   * Union is limited to the time complexity of the Find operation, as all the
   * other operations take constant time. Due to the improvement of making this
   * operation Weighted, the Find operation is sped up to a time complexity of
   * O(logn), thus making this implementation of Union also O(logn), compared to
   * the O(n) time complexity for the normal Union.
   *
