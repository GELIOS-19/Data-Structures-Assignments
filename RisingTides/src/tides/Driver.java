...
public class Driver implements MouseMotionListener {
  ...
  private void setCoordinates(final String text) {
    SwingUtilities.invokeLater(() -> { coordinates.setText(text); });
  }

  private void setElevationExtrema(final String text) {
    SwingUtilities.invokeLater(() -> { elevationExtremaLine.setText(text); });
  }

  private void setIsFlooded(final String text) {
    SwingUtilities.invokeLater(() -> { isFloodedLine.setText(text); });
  }

  private void setHeightAboveWater(final String text) {
    SwingUtilities.invokeLater(() -> { heightAboveWaterLine.setText(text); });
  }

  private void setTotalLand(final String text) {
    SwingUtilities.invokeLater(() -> { totalLandLine.setText(text); });
  }

  private void setLandLost(final String text) {
    SwingUtilities.invokeLater(() -> { landLostLine.setText(text); });
  }

  private void setNumOfIslands(final String text) {
    SwingUtilities.invokeLater(() -> { numOfIslandsLine.setText(text); });
  }
   ...
   * https://stackoverflow.com/questions/10985734/java-swing-enabling-
   * disabling- all-components-in-jpanel
   */
  ...
  private void runSimulation(File terrainFile) {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          window, "Please enter a number for the water height.", "Water Height",
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          window, "Please enter a number for the future water height.",
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          window, "Please enter a valid integer for the row (y) number.",
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          window, "Please enter a valid integer for the column (x) number.",
      ...
      public void run() {
        try {
          ...
          try {
            ...
            setElevationExtrema(
                "<html><b> &nbsp Elevation Extrema:</b>" + spacing +
          ...
          try {
            ...
            boolean isFlooded = rt.isFlooded(
            ...
            setIsFlooded("<html><b> &nbsp Is (" + isFloodedCol + ", " +
                         isFloodedRow + ") Flooded:</b><font color = 'red'>" +
                         spacing + res + "</font></html>");
          } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(
                window,
          ...
          try {
            double heightAboveWater = rt.heightAboveWater(
                waterHeight, new GridLocation(isFloodedRow, isFloodedCol));
            String outputHAW =
                heightAboveWater < 0 ? "meters below" : "meters above";
          ...
          try {
            ...
            setTotalLand("<html><b> &nbsp Total Land:</b><font color = 'red'>" +
                         spacing + totalLand +
          ...
          SwingUtilities.invokeLater(() -> { setEnabled(controlPanel, true); });
        }
      }
    }.start();
  }
