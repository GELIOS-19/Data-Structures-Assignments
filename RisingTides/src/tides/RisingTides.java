...
public class RisingTides {
   ...
   * than that of the neighboring terrain, flood the cells. Repeat iteratively till the neighboring
   * terrain is higher than the water height.
   *
  ...
  public boolean[][] floodedRegionsIn(double height) {
    ...
    while (floodedCells.size() != 0) {
      ...
      for (int[] direction : DIRECTIONS) {
        GridLocation neighbor = new GridLocation(curr.row + direction[0], curr.col + direction[1]);
        try {
   ...
   *
   * <p>If the return value is negative, the Driver will display "meters below" If the return value
   * is positive, the Driver will display "meters above" The value displayed will be positive.
   *
   * @param height of the water
   * @param cell location
   ...
   *
   * <p>If the return value is negative, the Driver will display "Will gain" If the return value is
   * positive, the Driver will display "Will lose" The value displayed will be positive.
   *
   * @param height of the water
    ...
    for (int row = 0; row < flooded.length; row++) {
      for (int col = 0; col < flooded[0].length; col++) {
        if (!flooded[row][col]) {
          ...
          for (int[] direction : DIRECTIONS) {
            GridLocation neighbor = new GridLocation(row + direction[0], col + direction[1]);
            try {
