...
public class RisingTidesVisualizer extends JPanel {
  ...
  private Color colorFor(int row, int col) {
    ...
    for (int i = 1; i < COLORS.length; i++) {
      if (alpha <= COLORS[i].threshold) {
        /*
         * Progress is measured by how far between the two points we are. 0.0
   ...
   * heights are mapped to real numbers between 0 and 1. Those real numbers are
   * then used to interpolate between a fixed series of color points marked off
   * at various intervals. The RGBPoint type represents a particular RGB color
   * annotated with a threshold value between 0 and 1 indicating where that
   * color sits.
   */
