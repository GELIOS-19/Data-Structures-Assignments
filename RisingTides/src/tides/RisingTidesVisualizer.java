...
public class RisingTidesVisualizer extends JPanel {
  ...
  private static final Color UNDERWATER_COLOR =
      new Color(0, 49, 83); // Prussian
                            // blue
  /* Background color */
  ...
  public void setFlooding(boolean[][] flooded) { this.flooded = flooded; }
  ...
  private Color colorFor(int row, int col) {
    ...
    for (int i = 1; i < COLORS.length; i++) {
      if (alpha <= COLORS[i].threshold) {
         ...
         */
        int red =
            (int)interpolate(progress, 0, 1, COLORS[i - 1].red, COLORS[i].red);
  ...
  public void paint(Graphics g) {
    ...
    for (int x = baseX; x < baseX + width; x++) {
      for (int y = baseY; y < baseY + height; y++) {
        int col =
            interpolate(x, baseX, (int)(baseX + width), 0, terrain[0].length);
        int row =
            interpolate(y, baseY, (int)(baseY + height), 0, terrain.length);
