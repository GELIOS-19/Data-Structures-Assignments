...
public class HungerGames {
   ...
   *
   * <p>Persons will be added to corresponding district in districts defined by
   * districtID
   *
   * @param filename will be provided by client to read from using StdIn
   */
  ...
  public void addDistrictToGame(TreeNode root, District newDistrict) {
    ...
    if (this.game == null) {
      ...
      } else if (newDistrict.getDistrictID() >
                 root.getDistrict().getDistrictID()) {
  ...
  public DuelPair selectDuelers() {
    ...
    this.findDistrict(oddDueler.getDistrictID())
        .getOddPopulation()
        .remove(oddDueler);
    this.findDistrict(evenDueler.getDistrictID())
        .getEvenPopulation()
        .remove(evenDueler);
  ...
  public void eliminateDistrict(int id) {
    ...
    // Case 2: One child
      ...
      TreeNode childNode =
          (target.getLeft() != null) ? target.getLeft() : target.getRight();
  ...
  public void eliminateDueler(DuelPair pair) {
    ...
    int winnerDistrictEvenPopulation =
        winnerDistrict.getEvenPopulation().size();
    ...
    int loserDistrictTotalPopulation =
        loserDistrictOddPopulation + loserDistrictEvenPopulation;

    if (winnerDistrictTotalPopulation == 0 ||
        winnerDistrictEvenPopulation == 0 || winnerDistrictOddPopulation == 0) {
   ...
   */
  public TreeNode getRoot() { return game; }
}
