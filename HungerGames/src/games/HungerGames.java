...
public class HungerGames {
    ...
    private TreeNode
            game; // root of the BST. The BST contains districts that are still in the game.

    /**
     * ***** DO NOT REMOVE OR UPDATE this method ********* Default constructor, initializes a list
     * of districts.
     */
    public HungerGames() {
    ...
    /**
     * Reads the following from input file: - Number of districts - District ID's (insert in order
     * of insertion) Insert districts into the districts ArrayList in order of appearance.
     *
     * @param filename will be provided by client to read from using StdIn
     */
     ...
     * setupDistricts()): Number of people Space-separated: first name, last name, birth month
     * (1-12), age, district id, effectiveness Districts will be initialized to the instance
     * variable districts
     *
    ...
    public void setupPeople(String filename) {
        ...
        for (int i = 0; i < numPeople; i++) {
            ...
            // Create the person
            Person person =
                    new Person(birthMonth, firstName, lastName, age, districtId, effectiveness);
    ...
    /**
     * Selects two duelers from the tree, following these rules: - One odd person and one even
     * person should be in the pair. - Dueler with Tessera (age 12-18, use tessera instance
     * variable) must be retrieved first. - Find the first odd person and even person (separately)
     * with Tessera if they exist. - If you can't find a person, use StdRandom.uniform(x) where x is
     * the respective population size to obtain a dueler. - Add odd person dueler to person1 of new
     * DuelerPair and even person dueler to person2. - People from the same district cannot fight
     * against each other.
     *
            ...
            if (oddDueler == null) {
                oddDueler =
                        node.getDistrict()
                                ...
                                .get(
                                        StdRandom.uniform(
                                                node.getDistrict().getOddPopulation().size()));
    ...
    /**
     * Eliminates a dueler from a pair of duelers. - Both duelers in the DuelPair argument given
     * will duel - Winner gets returned to their District - Eliminate a District if it only contains
     * a odd person population or even person population
     *
    ...
    public void eliminateDueler(DuelPair pair) {
        ...
        int winnerDistrictTotalPopulation =
                winnerDistrictOddPopulation + winnerDistrictEvenPopulation;
